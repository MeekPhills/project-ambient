...
PR #44 delivered and merged (`ca975278b4195d88c2d19ae3b995611665fee9d6`) the first #37 slice: provenance-preserving copy/reference import behind the typed command contract, with request-ID idempotency, rollback of created copies, fail-closed `privateReference` rights defaults, and 4/4 MediaImporterTests. The second slice — accessible Now/Next/Why narration, the import report card (reachable from onboarding and dashboard), assistive-technology announcements, and the apps/macos README — is on PR #48 on `feat/37-static-onboarding-import`, which also carries the fix round from its independent review (empty-library report reachability, stale-report lifecycle, issue-row identity, zero-import framing, Low Power Mode double narration, spoken-summary element, capped issue list) **and the re-run merge-gate fixes for slice-1 regressions found by the independent PR #44 review**: rescan no longer mis-tags images as "video"; import path-dedupes against provenance-less legacy assets (whose duplicate paths previously made every rescan crash); the scanner tolerates duplicate paths in stored state; imported videos carry the scanner's "video" tag; copy failures are reported as copy failures rather than "unreadable"; a zero-import reference folder is registered for future rescans; and `ambientctl import` rejects flag-first invocations with a usage error. Two of those deferred items — capping the request-ledger's stored import reports and content-aware rescan — plus the issue's injected-failure restore criterion are delivered on `feat/37-import-integrity`: rescan drops content duplicates of provenance-bearing assets (size-prefiltered so a clean rescan stays cheap), ledger entries keep a bounded 20-row excerpt while preserving exact counts, and tests cover injected import failure and injected apply failure both restoring last-known-good in memory and on disk. Still deferred with owner visibility: moving hashing/copying off the main actor and out of the exclusive state lock, and the owner decision on re-enabling or surfacing the now-dormant Vision auto-tagging. Remaining #37 criteria after this: clean-account smoke and the base-M4 benchmark trace plus privacy report under `docs/reports/`. After PR #48 merges, the remaining #37 units are: clean-account smoke, injected-failure restore coverage at the UI layer, and the base-M4 benchmark trace and privacy report under `docs/reports/`. No continuous renderer or model service; #37 closes only when every acceptance criterion has evidence.
