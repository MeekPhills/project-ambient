...
- Issue #37 remains the only integration-active issue; its Mac-local importer worktree and the protected checkout were untouched by this work. Next exact action for the program: implement issue #37's smallest safe unit in its existing worktree `../project-ambient-m1-onboarding-import` on `feat/37-static-onboarding-import`.
