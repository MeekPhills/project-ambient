...
**Parent baseline for this handoff:** `8d2d08a476c03df9f0a0ba1e1366c7db2087ea4b` on `main`, including merged PR #61.

**Working branch:** `feat/28-runtime-event-signposts` in `~/Developer/project-ambient`; the protected checkout is untouched. The historically specified #37 isolated worktree is not currently present or registered, so do not recreate or reconstruct it; its remaining product evidence is blocked pending an owner-directed safe workspace decision.
