...
**Parent baseline for this handoff:** `54748354e6a4121ad11c56679e3192606f970797` on `main`, including merged PR #60.

**Working branch:** `feat/28-wakeup-attribution` in `~/Developer/project-ambient`; the protected checkout is untouched. The historically specified #37 isolated worktree is not currently present or registered, so do not recreate or reconstruct it; its remaining product evidence is blocked pending an owner-directed safe workspace decision.
...
PR #55 merged as `a19bab4` from `feat/28-static-envelope-smoke`, adding a public-tool harness at `script/measure_m4_static_smoke.sh` and a deliberately non-qualifying report at `docs/reports/m4-base-m4-static-envelope-smoke.md`. PR #57 merged as `9843e13`, adding the versioned base-M4 resource-budget schema, fixture, fail-closed validator, and release/CI wiring. PR #58 merged as `0d6b8a1`, binding the smoke output to that fixture and emitting explicit partial coverage. PR #59 merged as `e0c55a5`, correcting the exact two-display fixture and adding sanitized topology capture; the observed host exposed only one online 1920×1080@120 Hz display, so the fixture was unmatched. PR #60 merged as `5474835`, adding a no-elevation public `proc_pid_rusage` helper after independent review corrected package-idle subset double-counting; its 60-second observation saw three interrupt wakeups (2.9997/min, above the 2/min ceiling). The current branch adds a bounded time-series collector and fail-closed summarizer. Three independent unprofiled five-minute windows observed 12 wakeups / 2.3653 per minute, 2 / 0.3931 per minute, and 0 / 0 per minute, with irregular timing and no stable cadence. Public profiler/log attempts did not attribute a user-space source; one System Trace attempt breached the prohibited-diagnostics stop condition, contributes no evidence, and was deleted. Decoder/GPU activity, frame pacing, pressure/failure behavior, and the 48-hour soak remain explicitly **unmeasured**. This is neither M4 certification nor tracker credit; issue #28 remains open.

## Next exact action

PR #44 and PR #48 delivered the first two #37 slices; PR #49 delivered import integrity and restore coverage; issue #50 / PR #52 delivered screen-lock-only rotation; PR #54 added an isolated clean-account smoke. Remaining #37 criteria are a real clean-account GUI smoke and the full base-M4 benchmark/dual-display/soak evidence. For #28, merge the bounded wakeup-series harness only after exact-head review and green CI, then add privacy-minimized signposts around existing lifecycle/display/power/clock/state/rotation handlers and correlate repeated unprofiled windows without treating profiler-induced activity as budget evidence. Decoder/GPU, frame pacing, pressure, and long-soak collection remain after that. No continuous renderer or model service; issue #37 remains open until every acceptance criterion has evidence.
