...
**Parent baseline for this handoff:** `1318e70aed850e7226a1e43b9b0e901c4f54249f` on `main`, including merged PR #65.

**Working branch:** `feat/28-static-wakeup-qualification-contract` in `~/Developer/project-ambient`; the protected checkout is untouched. The historically specified #37 isolated worktree is not currently present or registered, so do not recreate or reconstruct it; its remaining product evidence is blocked pending an owner-directed safe workspace decision.
...
PR #66's initial head `827723f222bb2a335313743b830503d8c2bda71a` passed the full aggregate and 8/8 CI, but independent review correctly withheld the merge: complete results were not bound to the frozen plan or fixed still, and a proof-complete window could be marked ineligible to hide it. The corrected contract keeps complete results inactive while the current plan's still digest is null; binds current incomplete results to the exact plan producer revision and SHA-256; requires the retained still to equal the plan; and makes eligibility the exact conjunction of retained proof in both schema and validator. Five positive and 88 fail-closed cases pass locally. The original green CI and review are superseded; fresh corrected-head aggregate, independent reviews, and CI are required before merge. No host run, coverage advance, issue closure, or tracker credit follows.

## Next exact action

PR #44 and PR #48 delivered the first two #37 slices; PR #49 delivered import integrity and restore coverage; issue #50 / PR #52 delivered screen-lock-only rotation; PR #54 added an isolated clean-account smoke. Remaining #37 criteria are a real clean-account GUI smoke and the full base-M4 benchmark/dual-display/soak evidence. For #28, the current bounded unit defines—but does not execute—a controlled settled-static wakeup qualification protocol: five fresh process instances of one exact candidate artifact, each with a five-minute warm-up and fifteen-minute signposts-off window, exact fixture/scenario binding, fixed fifteen-minute normalization, and nearest-rank P95 rank five. The existing resource fixture and coverage remain unchanged, the exploratory 301-sample window remains ineligible, and collector/preflight implementation plus a fixed non-personal still digest remain later prerequisites before any evidence run. Frame pacing, pressure/failure behavior, the exact dual-display fixture, release-candidate provenance, and long-soak collection remain after that. No continuous renderer or model service; issue #37 remains open until every acceptance criterion has evidence.
