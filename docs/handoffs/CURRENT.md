...
Branch `chore/36-site-dependency-policy` starts at merged PR #68 and is open as PR #69. Exact compatible dependency updates and two narrow tested overrides reduce all three governed workspaces to zero advisories: the site moves from 15 high / 4 moderate / 1 low to zero, MCP remains at zero, and release-only MCPB tooling moves from one high plus four propagated low package records to zero. All three workspaces carry exact `ignore-scripts=true` configuration; seven lockfile entries with install scripts are inventoried and disabled with no exceptions. Independent review rejected earlier heads for false, abbreviated, aliased, quoted, escaped, path-qualified, variable, programmatic, and folded-multiline command bypasses; incomplete lock/config discovery; inactive gate wiring; composite-action and source-trigger omissions; case-folded shrinkwrap escape; ambiguous audit suppression; residual-identity cardinality; line-ending drift; an unstated Windows packaging boundary; and inherited npm credentials. The corrected tree scans workflow and composite shell steps, shell/documentation sources, and repository JavaScript/TypeScript outside generated trees; case-folds reserved npm filenames; rejects relevant symlinks and shrinkwrap; requires one exact effective deny flag; prohibits transient execution and root lifecycle hooks; validates live audits by complete advisory identity; pins the active gate and MCPB builder bytes; enforces LF-governed inputs; and runs MCPB installation/packing inside an isolated allowlisted environment. The validator carries 95 fail-closed tamper cases and runs in both the aggregate and release-integrity workflow. Clean deny-script installs, site build plus 12/12 rendered tests, Drizzle check/generation, MCP 95-pass/one-expected-skip tests, isolated MCPB packing under a hostile parent npm environment, and all three zero-result audit thresholds pass locally; exact-head aggregate, fresh independent rereview, and CI remain before merge. Native Windows `cmd.exe` packaging is not claimed; Bash on macOS/Linux or WSL/Git Bash is required. The optional site lint script still exposes fourteen pre-existing anchor-rule findings in untouched files and is not hidden or broadened into this branch. This sidecar changes no tracker arithmetic, milestone state, or product claim.
