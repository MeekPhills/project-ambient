...
Branch `chore/36-site-dependency-policy` starts at merged PR #68 and is open as PR #69. Exact compatible dependency updates and two narrow tested overrides reduce the site audit from 15 high / 4 moderate / 1 low to one explicit low Babel finding; MCP remains at zero, and release-only MCPB tooling moves from one high plus four propagated low package records to zero. All three workspaces now carry exact `ignore-scripts=true` configuration; seven lockfile entries with install scripts are inventoried and disabled with no exceptions. `script/validate_npm_supply_chain.mjs` binds all three lock digests, fails closed on inventory/policy/tracker drift, and runs in the aggregate and release-integrity workflow. Clean deny-script installs, site build plus 12/12 rendered tests, Drizzle check/generation, MCP 95-pass/one-expected-skip tests, MCPB packing, audit thresholds, and the full release aggregate pass. The optional site lint script still exposes fourteen pre-existing anchor-rule findings in untouched files and is not hidden or broadened into this branch. Exact-head independent security review and CI remain before merge. This sidecar changes no tracker arithmetic, milestone state, or product claim.
