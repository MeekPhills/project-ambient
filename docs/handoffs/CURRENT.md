...
**Parent baseline for this handoff:** `e3b47db619eb76851d7bd2162bd77b2bb36846ab` on `main`, including merged PR #62.

**Working branch:** `feat/28-signpost-correlation-sanitizer` in `~/Developer/project-ambient`; the protected checkout is untouched. The historically specified #37 isolated worktree is not currently present or registered, so do not recreate or reconstruct it; its remaining product evidence is blocked pending an owner-directed safe workspace decision.
...
PR #55 merged as `a19bab4` from `feat/28-static-envelope-smoke`, adding a public-tool harness at `script/measure_m4_static_smoke.sh` and a deliberately non-qualifying report at `docs/reports/m4-base-m4-static-envelope-smoke.md`. PR #57 merged as `9843e13`, adding the versioned base-M4 resource-budget schema, fixture, fail-closed validator, and release/CI wiring. PR #58 merged as `0d6b8a1`, binding the smoke output to that fixture and emitting explicit partial coverage. PR #59 merged as `e0c55a5`, correcting the exact two-display fixture and adding sanitized topology capture; the observed host exposed only one online 1920×1080@120 Hz display, so the fixture was unmatched. PR #60 merged as `5474835`, adding a no-elevation public `proc_pid_rusage` helper after independent review corrected package-idle subset double-counting; its 60-second observation saw three interrupt wakeups (2.9997/min, above the 2/min ceiling). PR #61 merged as `8d2d08a` after a five-finding review/fix round, adding an identity-bound, exact-count wakeup series and recording three unprofiled five-minute windows at 12 / 2 / 0 wakeups with no stable cadence or P95 claim. PR #62 merged as `e3b47db` after independent exact-head approval and 8/8 green checks, adding opt-in payload-free signposts at eleven existing runtime seams with no new timer or polling. The current branch adds a fail-closed post-hoc sanitizer/correlator that retains only static names and relative buckets and refuses loss, payloads, unknown names, malformed input, or incomplete windows. A six-snapshot production-path smoke retrieved the launch marker with no loss and retained no in-window signposts; its three wakeup buckets/four interrupt wakeups had no nearby instrumented callback. Raw NDJSON never touched disk, the temporary process series was deleted, no correlation artifact was written, and this instrumented smoke is not budget evidence. Public profiler/log attempts did not attribute a user-space source; one earlier System Trace attempt breached the prohibited-diagnostics stop condition, contributes no evidence, and was deleted. Decoder/GPU activity, frame pacing, pressure/failure behavior, and the 48-hour soak remain explicitly **unmeasured**. This is neither M4 certification nor tracker credit; issue #28 remains open.

## Next exact action

PR #44 and PR #48 delivered the first two #37 slices; PR #49 delivered import integrity and restore coverage; issue #50 / PR #52 delivered screen-lock-only rotation; PR #54 added an isolated clean-account smoke. Remaining #37 criteria are a real clean-account GUI smoke and the full base-M4 benchmark/dual-display/soak evidence. For #28, review and merge the sanitizer only after its deterministic tamper suite, full release aggregate, independent exact-head review, and green CI. Then run one bounded signposts-on correlation window through that sanitizer and compare repeated matched signposts-off windows; only uninstrumented windows may count toward the budget. The next implementation unit is sampled physical footprint with fail-closed partial assertion semantics. Decoder/GPU, frame pacing, pressure, and long-soak collection remain after that. No continuous renderer or model service; issue #37 remains open until every acceptance criterion has evidence.
