...
**Program state:** M0 is complete at 8/8. The canonical tracker remains publicly verified at 20/100 on schema v3; no M1, M4, or security-sidecar credit is awarded. Issue #37 remains open for its outstanding evidence, issue #28 has several merged but deliberately partial base-M4 static-envelope evidence slices, and issue #36 is in a bounded dependency-policy remediation.

**Repository:** https://github.com/MeekPhills/project-ambient

**Parent baseline for this handoff:** `84ced53b980961b9db7e62daa852e45d522127de` on `main`, including merged PR #68.

**Working branch:** `chore/36-site-dependency-policy` in `~/Developer/project-ambient`; the protected checkout is untouched. The historically specified #37 isolated worktree is not currently present or registered, so do not recreate or reconstruct it; its remaining product evidence is blocked pending an owner-directed safe workspace decision.
...
PR #67 merged as `8d95a63e18e913c1c04a8378fc2b2d317b503567` after the full aggregate, 8/8 exact-head CI, and three independent exact-head approvals. Source-producing revision `b439193ed513811bec90ce2491ec30033aa2a4e4` adds a closed automated-preflight schema, a native pure evaluator, strict Draft 2020-12 checks, and release/CI wiring; the final binding requires that exact revision. The active schema accepts only the current null-still stop and invokes zero host callbacks; future checked-stop/pass forms remain inactive synthetic definitions, and the production host-fact callback remains unavailable. The later PR #68 collector branch leaves that 15-positive/78-fail-closed active validator unchanged and adds a strict engine with twelve positive and 49 fail-closed synthetic cases. Exact-head review rejected the first collector commit because alternate-plan/result validators were exported, stalled adapter operations could bypass cleanup, termination was requested without proven exit, and launcher-return ownership was not atomic. The corrected tree keeps alternate plans, complete results, and execution module-private; exports only an exact active-null-plan constructor; uses a collector-minted synchronous ownership receiver and unforgeable acknowledgement before handle return; bounds every host operation with collector-owned timeouts and abort signals; binds measurement and cleanup to the launcher identity; and requires confirmed identity-bound exit with forced escalation before any later trial. All six owner attestations remain closed inputs. Its production entry point emits no result and stops before preflight, attestation, adapter, or launch. Those merged slices create no host observation, collection authority, qualification evidence, coverage, or tracker credit. A collection-ready public host adapter remains gated on the fixed-still rights/delivery decision and activated plan bindings.

PR #68 merged as `84ced53b980961b9db7e62daa852e45d522127de` from reviewed head `04fde57d32c3a34518945b58f2a5cc705b67c7fb` after the full aggregate passed twice, 8/8 exact-head CI passed, and two fresh independent reviewers approved. The strict collector remains synthetic-only and inactive: no host run, result artifact, qualification evidence, coverage, issue state, or tracker credit changed. A pickup review found one authority-free correctness follow-on: valid automated checked-stop reasons are currently collapsed to generic `preflight-invalid` by the collector. That synthetic handoff closure can preserve all eleven exact mismatch reasons without enabling host callbacks; it follows the security sidecar below.

## Issue #36 npm security checkpoint (2026-08-23)

Branch `chore/36-site-dependency-policy` starts at merged PR #68 and is open as PR #69. Exact compatible dependency updates and two narrow tested overrides reduce the site audit from 15 high / 4 moderate / 1 low to one explicit low Babel finding; MCP remains at zero, and release-only MCPB tooling moves from one high plus four propagated low package records to zero. All three workspaces carry exact `ignore-scripts=true` configuration; seven lockfile entries with install scripts are inventoried and disabled with no exceptions. Independent review rejected earlier head `ff2b45e72ff473ecff76a14e2470e69d2f36af51`: operational command parsing admitted false/wrapped forms, root/hidden/symlink locks and higher-precedence shrinkwrap could escape discovery, workflow-only changes could skip the gate, release `npm pack` could run hooks, the MCPB convenience command used unversioned `npx`, install-time audit reporting was suppressed, contributor instructions were unguarded, and the retained low advisory identity was not reconciled to live output. Later rereview rejected negated, continued, and bare-flag-plus-false forms, missing script-source triggers, wrapped/variable/programmatic npm execution, install-test/ci aliases, and a pre-pack proof that comments or dead code could satisfy. The corrected tree discovers source and tracked inputs repository-wide, rejects shrinkwrap and relevant symlinks, requires one effective true deny flag across every known lifecycle-capable npm operation and alias, prohibits transient/wrapped execution, root lifecycle hooks, and `npx`, uses only the locked MCPB builder, removes audit suppression, broadens release triggers, and validates fresh audit JSON exactly by counts plus GHSA/package/node/workspace identity. Release `npm pack` retains the explicit deny flag; the exact active policy-validator sequence immediately before it is the sole pack command, and the reviewed release packager bytes are hash-pinned so dead-branch or extra-pack drift fails closed. The validator carries 63 fail-closed tamper cases and runs in both the aggregate and release-integrity workflow. Clean deny-script installs, site build plus 12/12 rendered tests, Drizzle check/generation, MCP 95-pass/one-expected-skip tests, MCPB packing, and audit thresholds passed before the review round; exact-head aggregate, independent rereview, and CI remain before merge. The optional site lint script still exposes fourteen pre-existing anchor-rule findings in untouched files and is not hidden or broadened into this branch. This sidecar changes no tracker arithmetic, milestone state, or product claim.

## Next exact action

Complete issue #36's exact-head non-author security review and CI, fix any real finding, then merge its zero-credit sidecar. Next, take the authority-free #28 synthetic checked-stop handoff closure: preserve each of the eleven automated preflight mismatch reasons end to end while proving zero attestation, adapter, launch, trial, cleanup, or result writes. The existing resource fixture and coverage remain unchanged. A collection-ready public host adapter, owner-approved still delivery class/rights disposition and manifest, reviewed fixed non-personal still digest, activated plan bindings, owner attestations, exact dual-display fixture, frame-pacing and pressure/failure evidence, release-candidate provenance, and long soak remain prerequisites for live #28 evidence. Issue #37's weighted frontier remains the real clean-account GUI smoke plus the full base-M4/dual-display/soak evidence in an owner-directed safe workspace. No continuous renderer or model service; issue #37 and #28 remain open until every criterion has evidence.
