...
- Issue #37 remains the only integration-active issue; its Mac-local importer worktree and the protected checkout were untouched by this work. Next exact action for the program: review and merge PR #44 — the first #37 slice (typed copy/reference import with provenance, commit `fcdbe7e`, exact-head CI green) — then continue #37's remaining acceptance criteria in the same worktree.
