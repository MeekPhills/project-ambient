...
The next bounded #28 prerequisite is in progress on `feat/28-static-wakeup-preflight-contract`. Source-producing commit `d97acf7d042a3138fae086e02f5aada2cfcc8116` adds a closed automated-preflight schema, a native pure evaluator, 15 positive and 77 fail-closed contract cases, strict Draft 2020-12 checks, and release/CI wiring; the follow-up binding requires that exact producer revision. The active schema accepts only the current null-still stop and invokes zero host callbacks; future checked-stop/pass forms remain inactive synthetic definitions, and the production host-fact callback remains unavailable. This branch creates no host observation, collection authority, qualification evidence, coverage, or tracker credit.
