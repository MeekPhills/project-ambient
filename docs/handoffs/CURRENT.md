...
- Issue #37 remains the only integration-active issue; its Mac-local importer worktree and the protected checkout were untouched by this work. PR #44 — the first #37 slice (typed copy/reference import with provenance, reviewed head `fcdbe7e`, exact-head CI green) — merged as `ca975278b4195d88c2d19ae3b995611665fee9d6` on 2026-08-16 with owner approval; #37 stays open with zero new credit.

## Next exact action

PR #44 delivered and merged (`ca975278b4195d88c2d19ae3b995611665fee9d6`) the first #37 slice: provenance-preserving copy/reference import behind the typed command contract, with request-ID idempotency, rollback of created copies, fail-closed `privateReference` rights defaults, and 4/4 MediaImporterTests. Continue #37's remaining acceptance criteria in the same `feat/37-static-onboarding-import` worktree, smallest safe unit first: accessible Now/Next/Why and import-report surfacing in the UI, clean-account smoke, injected-failure restore coverage at the UI layer, the base-M4 benchmark trace and privacy report under `docs/reports/`, and the apps/macos README and handoff updates. No continuous renderer or model service; #37 closes only when every acceptance criterion has evidence.
